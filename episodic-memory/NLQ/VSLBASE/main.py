...
def main(configs, parser):
    ...
    if configs.mode.lower() == "train":
        ...
        if configs.pretrained_weights:
            print(f"Caricamento pesi pre-addestrati da {configs.pretrained_weights}")
            model.load_state_dict(torch.load(configs.pretrained_weights), strict=False)
            ...
            for data in tqdm(
                ...
                if (
                    ...
                    if results[0][0] >= best_metric:
                        ...
                        filter_checkpoints(model_dir, suffix="t7", max_to_keep=1000)
                    else:
                        torch.save(
                            model.state_dict(),
                            os.path.join(
